...
| `zai_usage.py` | Reads `ZAI_API_KEY` env var, calls `https://api.z.ai/api/monitor/usage/quota/limit` (key passed directly in `Authorization`, no `Bearer`) for 5-hour/weekly token windows and monthly MCP usage |
...
| `ZAI_API_KEY` | Optional | Z.ai GLM Coding Plan API key from `https://z.ai/manage-apikey/apikey-list` |
